...
TABLE = config_parser.get("gcp_variables", "table")
...
CREATE_TBL_QUERY = config_parser.get("sql_scripts", "create_tbl_query")
...
def main(bq):
    ...
    create_bq_dataset(
        ...
        region=LOCATION,
        description="dataset is created for new york citibikes trips prediction",
        log=log,
    )
    log.info("dataset created.")

    log.info("creating table for training data")
    train_sql = get_bq_query(CREATE_TBL_QUERY, log)
    create_bq_table(bq, train_sql, PROJECT, DATASET, TABLE, LOCATION, log=log)
    log.info("table created.")

    log.info("starting ARIMA model training")
    train_sql = get_bq_query(TRAIN_QUERY, log)
    training_sql = train_sql.format(PROJECT, DATASET, MODEL, TABLE)
    ...
    train_info_query = train_info.format(PROJECT, DATASET, MODEL, TABLE)
    execute_query_output(
    ...
    forecast_sql = get_bq_query(PRED_QUERY, log)
    forecasting_sql = forecast_sql.format(PROJECT, DATASET, MODEL)
    execute_query_output(
        client=bq, query=forecasting_sql, location=LOCATION, name=PRED_FILE, log=log
    )
